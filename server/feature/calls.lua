...
lib.callback.register('z-phone:server:StartCall', function(source, body)
    ...
    local contactNameQuery = [[
        SELECT zpc.contact_name FROM zp_contacts zpc WHERE zpc.phone_number = ? AND zpc.contact_phone_number = ?
    ]]
    ...
    local historyQuery = "INSERT INTO zp_calls_histories (phone_number, to_phone_number, flag, is_anonim) VALUES (?, ?, ?, ?)"
...
lib.callback.register('z-phone:server:GetCallHistories', function(source)
    ...
    local query = [[
        ...
        LEFT JOIN zp_users zpu ON zpu.phone_number = zpch.to_phone_number
        LEFT JOIN zp_contacts zpc ON zpc.contact_phone_number = zpch.to_phone_number AND zpc.phone_number = zpch.phone_number
        WHERE zpch.phone_number = ? ORDER BY zpch.id DESC LIMIT 100
    ]]
