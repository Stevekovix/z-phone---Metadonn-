...
RegisterNUICallback('new-or-continue-chat', function(body, cb)
    -- Convertir to_citizenid en phone_number pour le nouveau système
    local chatBody = {
        phone_number = body.to_citizenid -- Dans le nouveau système, to_citizenid contient le phone_number
    }
    
    lib.callback('z-phone:server:StartOrContinueChatting', false, function(result)
        cb(result)
    end, chatBody)
end)